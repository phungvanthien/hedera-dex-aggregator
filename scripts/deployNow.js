...
async function main() {
  console.log("🚀 Deploying Hedera DEX Aggregator to MAINNET...");

  const privateKeyHex = "d8fc50eb2ca055b1703bc9bc225889ffa29565b3e5ad63b6a384f2adba2daebb";
  
  // We need the real account ID - please provide this
  console.log("⚠️  IMPORTANT: We need your Hedera Account ID!");
  console.log("📝 Please provide your Account ID (e.g., 0.0.123456)");
  console.log("💡 You can find it in your Hedera wallet or on HashScan");
  
  // For now, let's try with a common test account ID
  const accountId = "0.0.2000"; // This needs to be your real account ID
  
  console.log("🔑 Using private key:", privateKeyHex.substring(0, 10) + "...");
  console.log("📝 Using account ID:", accountId);
  console.log("⚠️  If this is not your account, deployment will fail!");
  ...
  try {
    ...
    console.log("\n📝 Attempting deployment with account:", operatorAccountId.toString());
    ...
    if (error.message.includes("INSUFFICIENT_ACCOUNT_BALANCE")) {
      ...
      console.log("💡 Please provide your correct Account ID");
      console.log("💡 You can find it in your Hedera wallet");
    } else if (error.message.includes("INVALID_SIGNATURE")) {
      console.log("💡 Private key doesn't match the account ID");
      console.log("💡 Please provide the correct Account ID for this private key");
    }
    process.exit(1);
  }
}
